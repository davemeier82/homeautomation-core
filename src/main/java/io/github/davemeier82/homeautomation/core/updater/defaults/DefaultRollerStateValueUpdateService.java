...
public class DefaultRollerStateValueUpdateService extends AbstractValueUpdateService<RollerState> implements RollerStateValueUpdateService {

  private final EventFactory eventFactory;

  public DefaultRollerStateValueUpdateService(DevicePropertyValueRepository devicePropertyValueRepository,
                                              DevicePropertyCreator devicePropertyCreator,
                                              EventPublisher eventPublisher,
                                              EventFactory eventFactory
  ) {
    super(devicePropertyValueRepository, devicePropertyCreator, eventPublisher, RollerState.class);
  ...
  @Override
  protected DevicePropertyEvent<RollerState> createUpdatedEvent(DevicePropertyId devicePropertyId,
                                                                DataWithTimestamp<RollerState> newValue,
                                                                DataWithTimestamp<RollerState> previousValue,
                                                                String displayName
  ) {
    return eventFactory.createRollerStateUpdatedEvent(devicePropertyId, newValue, previousValue, displayName);
  }

  @Override
  protected DevicePropertyEvent<RollerState> createChangedEvent(DevicePropertyId devicePropertyId,
                                                                DataWithTimestamp<RollerState> newValue,
                                                                DataWithTimestamp<RollerState> previousValue,
                                                                String displayName
  ) {
    return eventFactory.createRollerStateChangedEvent(devicePropertyId, newValue, previousValue, displayName);
  }
